package com.velora.novabox.app

import android.content.Context
import android.content.SharedPreferences
import android.util.Log
import com.velora.novabox.util.InjectionUtil
import com.velora.novabox.view.main.BlackBoxLoader
...
object AppManager {
    ...
    fun doOnCreate(context: Context) {
        try {
            ...
            if (mBlackBoxCore.isMainProcess) {
    ...
    private fun preloadInstalledApps() {
        Thread {
            try {
                InjectionUtil.preloadInstalledApps()
                Log.d(TAG, "Installed app list preloaded")
            } catch (e: Exception) {
                Log.e(TAG, "Error preloading installed apps: ${e.message}")
            }
        }.apply {
            name = "InstalledAppsPreloader"
            isDaemon = true
            start()
        }
    }
